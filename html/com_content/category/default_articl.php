...
?>
				<div class="row mb-4">
					...
					<div class="col-md-3"><a href="#" class="zakaz-element ba-click-lightbox-form-3"><img src="/images/raschet-w.svg" title="заказать расчет!"> заказать расчет!</a><a href="/kalkulyatory" class="zakaz-element zakaz-element-calc"><img src="/images/calc-w.svg" title="Калькуляторы"> Калькулятор расчета</a></div>
				</div>
		...
		<?php if (empty($this->items)) : ?>
			...
			<?php else : ?>
				...
				<?php foreach ($this->items as $i => $article) : ?>
					<div class="col-md-4 float-left item_proekt">
						<div class="shadow">
							<a href="<?php echo JRoute::_(ContentHelperRoute::getArticleRoute($article->slug, $article->catid, $article->language)); ?>">
								<? 
								$params_article = array();
								foreach ($article->jcfields as $parm) {
									$params_article[$parm->name] = $parm->rawvalue;
								} ?>
								<? $img = json_decode($article->images); ?>
								<div><img src="<?=$img->image_fulltext?>"></div>
								<div class="name-block">
									<div class="name mb-2">
										<?php echo $this->escape($article->title); ?>
									</div>
									<? if(!empty($params_article['podpis-dlya-prevyu-stati'])){?><div class="desc_it"><?= $params_article['podpis-dlya-prevyu-stati'];?></div><? } ?>
									<? if(!empty($params_article['obem-postavki'])){?><div class="col_it">Объем поставки: <?= $params_article['obem-postavki'];?></div><? } ?>
								</div>
							</a>
						</div>
					</div>
