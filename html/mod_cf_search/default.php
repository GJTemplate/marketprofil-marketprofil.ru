...
<form class="cf-form-search" id="cf_form_<?php echo $module->id ?>" role="search"
      action="<?php echo JRoute::_('index.php?option=com_customfilters&view=products&Itemid=' . $itemId) ?>"
      method="get">
    <div class="cf-searchmod-wrapper<?php echo $suffix; ?>" id="cf_wrapp_all_<?php echo $module->id ?>">
		<span class="input-append">
			<input name="q" id="q_<?php echo $module->id ?>_0" value="<?php echo $value ?>" type="search"
                   placeholder="<?php echo JText::_('MOD_CF_SEARCH_INPUT_PLACEHOLDER') ?>" maxlength="100"
                   aria-label="<?php echo JText::_('MOD_CF_SEARCH_SEARCH') ?>"
                   size="<?php echo $size ?>" class="cf-searchmod-input  <?php echo $input_class ?>"/>

			<button type="submit" id="q_<?php echo $module->id ?>_button"
                    class="btn btn-primary cf_apply_button <?php echo $btn_class ?>"
                    title="<?php echo JText::_('MOD_CF_SEARCH_SEARCH') ?>"><?php echo JText::_('MOD_CF_SEARCH_SEARCH') ?></button>
		</span>
