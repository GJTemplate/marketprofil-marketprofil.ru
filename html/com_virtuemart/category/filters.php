...
jQuery(document).ready(function () {
	...
	?>
	<?if(!empty($this->category->params_30)){ ?><div class="row"><div class="col-md-9"><?=htmlspecialchars_decode($this->category->params_30)?></div><div class="col-md-3"><a href="#" class="zakaz-element ba-click-lightbox-form-3"><img src="/images/raschet-w.svg" title="заказать расчет!"> заказать расчет!</a><a href="/kalkulyatory" class="zakaz-element zakaz-element-calc"><img src="/images/calc-w.svg" title="Калькуляторы"> Калькулятор расчета</a></div></div><? } ?>	<?
...
if(!empty($this->orderByList)) { ?>
	<div class="orderby-displaynumber">
			...
			<? 
			if(stristr($_SERVER['REQUEST_URI'], '&orderby=product_price&order=DESC') === FALSE) {
				?>
				...
				<a href="<?= str_replace(array("/dirDesc?keyword=", "&orderby=product_price&order=DESC", "&tmpl=component"), "", $_SERVER['REQUEST_URI'])?>&orderby=product_price&order=DESC">Дорогие</a>
				<?
			}
			else{
				?>
				<a href="<?= str_replace(array("/dirDesc?keyword=", "&orderby=product_price&order=DESC", "&tmpl=component"), "", $_SERVER['REQUEST_URI'])?>" >Дешевые</a>
				<a href="<?= str_replace(array("/dirDesc?keyword=", "&orderby=product_price&order=DESC", "&tmpl=component"), "", $_SERVER['REQUEST_URI'])?>&orderby=product_price&order=DESC" class="active">Дорогие</a>
				<?
			}
			?>
