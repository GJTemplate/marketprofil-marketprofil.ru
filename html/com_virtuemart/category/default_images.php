				...
				if (!empty($this->product->images)) {
					...
					<div class="main-image">
						...
						if(!empty($width) or !empty($height)){
							//echo $image->displayMediaThumb("",true,"rel='vm-additional-images' title=''", true, true, false, $width, $height);
							$name = explode(" (", $this->product->product_name);
							...
							$query->where($db_1->quoteName('virtuemart_product_id') . ' = ' . $db_1->quote($this->product->virtuemart_product_id));
							$db_1->setQuery($query);
							...
							<a title="<?=$text?> <?=$name[0]?> <?=$tol_1;?> <?=$pokr_1?>" rel="vm-additional-images" href="<?=$image->file_url?>"><img src="<?=$image->file_url?>" title="<?= $this->product->product_name?>" alt="<?=$text?> <?=$name[0]?> <?=$tol_1;?> <?=$pokr_1?>"></a>
							<?
						} else {
							echo $image->displayMediaFull("",true,"rel='vm-additional-images' title=''");
						}
						?>
						<div class="clear"></div>
					</div>
