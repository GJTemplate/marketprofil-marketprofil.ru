		...
		<div class="spacer-buy-area pd0">
			...
			<div class="col-md-12" id="colors_link">
				...
				foreach ($results as $value) {
					...
					$query->select(array('a.*', 'b.published'));
					$query->from($db_name->quoteName('#__virtuemart_products_ru_ru', 'a'));
					...
					$query->where($db_name->quoteName('a.virtuemart_product_id') . ' = ' . $db_name->quote($value->virtuemart_product_id));


					$db_name->setQuery($query);
					$results_name = $db_name->loadObjectList();
					$mass_res[] = array('name'=>$results_name[0]->product_name,'id'=>$value->virtuemart_product_id, 'tolshcina'=>$tol_1, 'pokritie'=>$pokr_1, 'color'=>$cvet_1, 'published'=>$results_name[0]->published);
				...
				foreach ($mass_res as  $value) {
					...
					if ($this_name[0] == $prod_name[0]) {
						if ($value['pokritie'] == $pokr && $value['tolshcina'] == $tol && $value['published'] == 1 ) {
			...
			<div class="col-md-12" id="tolshchina">
				...
				foreach ($mass_res as  $value) {
					...
					$prod_name = explode(" (", $value['name']);
					if ($this_name[0] == $prod_name[0]) {
						if ($value['pokritie'] == $pokr && $value['color'] == $cvet && $value['published'] == 1 ) {
							$link = JRoute::_('index.php?option=com_virtuemart&view=productdetails&virtuemart_product_id=' .$value['id'] , FALSE);
							?>
			...
			<div class="row abs_btn">
				...
				<div class="product-price"  data-vm="product-prices">
					...
					<div class="PricesalesPrice vm-display vm-price-value"><span class="PricesalesPrice"><?=round($this->product->allPrices[0]['salesPrice']);?> руб/м<sup>2</sup></span></div></div>
				</div>
...
<div class="row">
	...
	<div class="tab-pane fade" id="tabs5" role="tabpanel" aria-labelledby="tabs5-tab">
		...
		if(!stristr($this->product->product_name, 'Ламонтерра (') === FALSE){	?>
			<img class=" jch-lazyloaded" title="Чертеж металлочерепицы Ламонтерра" alt="Схема металлочерепица Монтеррей фото" src="/images/foto-kat/lamonterra.jpg" >
		<?	}
		elseif(!stristr($this->product->product_name, 'Ламонтерра X (') === FALSE){	?>
			<img class=" jch-lazyloaded" title="Чертеж металлочерепицы Ламонтерра Х" alt="Схема металлочерепица Супермонтеррей Х фото" src="/images/foto-kat/lamonterra X.jpg" >
		<?	}
		elseif(!stristr($this->product->product_name, 'Ламонтерра-XL (') === FALSE){	?>
			<img class=" jch-lazyloaded" title="Чертеж металлочерепицы Ламонтерра-XL" alt="Схема металлочерепица Макси фото" src="/images/foto-kat/maksi.jpg" >
		<?	}
		elseif(!stristr($this->product->product_name, 'Трамонтана') === FALSE){	?>
			<img class=" jch-lazyloaded" title="Чертеж металлочерепицы Трамонтана" alt="Схема металлочерепица Трамонтана фото" src="/images/foto-kat/tramontana.jpg" >
		<?	}
		elseif(!stristr($this->product->product_name, 'Монтекристо') === FALSE){	?>
			<img class=" jch-lazyloaded" title="Чертеж металлочерепицы Монтекристо" alt="Схема металлочерепица Монтекристо фото" src="/images/foto-kat/montekristo.jpg" >
		<?	}	
		elseif(!stristr($this->product->product_name, 'Монтерроса') === FALSE){	?>
			<img class=" jch-lazyloaded" title="Чертеж металлочерепицы Монтерроса" alt="Схема металлочерепица Монтерроса фото" src="/images/foto-kat/monterrosa.jpg" >
		<?	}			
		else{	?>
			<img class=" jch-lazyloaded" title="<?= $this->product->product_name;?>">
		<?	}
		?>
	</div>
