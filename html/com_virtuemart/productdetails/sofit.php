...
foreach ($this->product->customfieldsSorted['normal'] as $value) {
	...
	$chast_1 = explode("Софит", $this->product->product_name);
	$chast_2 = explode(" (", $chast_1[1]);
$app = JFactory::getApplication();
$document = JFactory::getDocument();
if ($this->product->customtitle) {
    $document->setTitle(strip_tags(html_entity_decode(ucfirst(strtolower($chast_2[0])).' софит '.$pokr.' '.$tol.' мм. '.$cvet.' | ⚒️ Металл Профиль',ENT_QUOTES)));
	$document->setDescription(strip_tags(html_entity_decode(ucfirst(strtolower($chast_2[0])).' софит в покрытии '.$pokr.' '.$tol.' мм. цвет '.$cvet.' ➡️ купить от Металл Профиль ✅ доступные цены ➡️ Качественный сервис ☎️ +7 (495) 259-24-19',ENT_QUOTES)));
} else {
    $document->setTitle(strip_tags(html_entity_decode(ucfirst(strtolower($chast_2[0])).' софит '.$pokr.' '.$tol.' мм. '.$cvet.' | ⚒️ Металл Профиль',ENT_QUOTES)));
	$document->setDescription(strip_tags(html_entity_decode(ucfirst(strtolower($chast_2[0])).' софит в покрытии '.$pokr.' '.$tol.' мм. цвет '.$cvet.' ➡️ купить от Металл Профиль ✅ доступные цены ➡️ Качественный сервис ☎️ +7 (495) 259-24-19',ENT_QUOTES)));
}
...
<div class="row">
	...
	<div class="tab-pane fade show active" id="tabs8" role="tabpanel" aria-labelledby="tabs8-tab">
		...
		if ($pokr == 'Полиэстер') {
				echo $this->loadTemplate('text27');
			}
			elseif ($pokr == 'ECOSTEEL®') {
				echo $this->loadTemplate('text28');
			}
			elseif ($pokr == 'PURMAN') {
				echo $this->loadTemplate('text29');
			}
			elseif ($pokr == 'VALORI') {
				echo $this->loadTemplate('text30');
			}
			elseif ($pokr == 'Викинг') {
				echo $this->loadTemplate('text31');
			}
			elseif ($pokr == 'Викинг Е') {
				echo $this->loadTemplate('text32');
			}			
		else{
