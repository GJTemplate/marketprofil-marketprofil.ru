...
if ($this->product->customtitle) {
    $document->setTitle(strip_tags(html_entity_decode(ucfirst(strtolower($chast_2[0])).' штакетник '.$pokr.' '.$tol.' мм. '.$cvet.' | ⚒️ Металл Профиль',ENT_QUOTES)));
	$document->setDescription(strip_tags(html_entity_decode(ucfirst(strtolower($chast_2[0])).' штакетник в покрытии '.$pokr.' '.$tol.' мм. цвет '.$cvet.' ➡️ купить от Металл Профиль ✅ доступные цены ➡️ Качественный сервис ☎️ +7 (495) 259-24-19',ENT_QUOTES)));
} else {
    $document->setTitle(strip_tags(html_entity_decode(ucfirst(strtolower($chast_2[0])).' штакетник '.$pokr.' '.$tol.' мм. '.$cvet.' | ⚒️ Металл Профиль',ENT_QUOTES)));
	$document->setDescription(strip_tags(html_entity_decode(ucfirst(strtolower($chast_2[0])).' штакетник в покрытии '.$pokr.' '.$tol.' мм. цвет '.$cvet.' ➡️ купить от Металл Профиль ✅ доступные цены ➡️ Качественный сервис ☎️ +7 (495) 259-24-19',ENT_QUOTES)));
}
...
<div class="row">
	...
	<div class="tab-pane fade show active" id="tabs8" role="tabpanel" aria-labelledby="tabs8-tab">
		...
		if (in_array('5175', $arr_cat_id)) {
				echo $this->loadTemplate('text21');
			}
			elseif (in_array('5176', $arr_cat_id)) {
				echo $this->loadTemplate('text22');
			}
			elseif (in_array('5177', $arr_cat_id)) {
				echo $this->loadTemplate('text23');
			}			
		else{
