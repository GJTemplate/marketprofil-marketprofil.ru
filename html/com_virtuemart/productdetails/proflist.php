...
foreach ($this->product->customfieldsSorted['normal'] as $value) {
	...
	$chast_1 = explode(" лист ", $this->product->product_name);
	$chast_2 = explode(" (", $chast_1[1]);
$app = JFactory::getApplication();
$document = JFactory::getDocument();
if ($this->product->customtitle) {
    $document->setTitle(strip_tags(html_entity_decode(ucfirst(strtolower($chast_2[0])).' профнастил '.$pokr.' '.$tol.' мм. '.$cvet.' | ⚒️ Металл Профиль',ENT_QUOTES)));
	$document->setDescription(strip_tags(html_entity_decode(ucfirst(strtolower($chast_2[0])).' профнастил в покрытии '.$pokr.' '.$tol.' мм. цвет '.$cvet.' ➡️ купить от Металл Профиль ✅ доступные цены ➡️ Качественный сервис ☎️ +7 (495) 259-24-19',ENT_QUOTES)));
} else {
    $document->setTitle(strip_tags(html_entity_decode(ucfirst(strtolower($chast_2[0])).' профнастил '.$pokr.' '.$tol.' мм. '.$cvet.' | ⚒️ Металл Профиль',ENT_QUOTES)));
	$document->setDescription(strip_tags(html_entity_decode(ucfirst(strtolower($chast_2[0])).' профнастил в покрытии '.$pokr.' '.$tol.' мм. цвет '.$cvet.' ➡️ купить от Металл Профиль ✅ доступные цены ➡️ Качественный сервис ☎️ +7 (495) 259-24-19',ENT_QUOTES)));
}
...
<div class="row">
	...
	<div class="tab-pane fade show active" id="tabs8" role="tabpanel" aria-labelledby="tabs8-tab">
		...
		if (in_array('5164', $arr_cat_id)) {
				echo $this->loadTemplate('text7');
			}
			elseif (in_array('5170', $arr_cat_id)) {
				echo $this->loadTemplate('text8');
			}
			elseif (in_array('5169', $arr_cat_id)) {
				echo $this->loadTemplate('text9');
			}
			elseif (in_array('5168', $arr_cat_id)) {
				echo $this->loadTemplate('text10');
			}
			elseif (in_array('5162', $arr_cat_id)) {
				echo $this->loadTemplate('text11');
			}
			elseif (in_array('5167', $arr_cat_id)) {
				echo $this->loadTemplate('text12');
			}
			elseif (in_array('5165', $arr_cat_id)) {
				echo $this->loadTemplate('text13');
			}
			elseif (in_array('5166', $arr_cat_id)) {
				echo $this->loadTemplate('text14');
			}
			elseif (in_array('5160', $arr_cat_id)) {
				echo $this->loadTemplate('text15');
			}
			elseif (in_array('5163', $arr_cat_id)) {
				echo $this->loadTemplate('text16');
			}
			elseif (in_array('5159', $arr_cat_id)) {
				echo $this->loadTemplate('text17');
			}			
		else{
