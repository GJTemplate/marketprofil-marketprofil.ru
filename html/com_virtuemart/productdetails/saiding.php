...
foreach ($this->product->customfieldsSorted['normal'] as $value) {
	...
	$chast_1 = explode("Сайдинг", $this->product->product_name);
	$chast_2 = explode(" (", $chast_1[1]);
$app = JFactory::getApplication();
$document = JFactory::getDocument();
if ($this->product->customtitle) {
    $document->setTitle(strip_tags(html_entity_decode(ucfirst($chast_2[0]).' сайдинг '.$pokr.' '.$tol.' мм. '.$cvet.' | ⚒️ Металл Профиль',ENT_QUOTES)));
	$document->setDescription(strip_tags(html_entity_decode(ucfirst($chast_2[0]).' сайдинг в покрытии '.$pokr.' '.$tol.' мм. цвет '.$cvet.' ➡️ купить от Металл Профиль ✅ доступные цены ➡️ Качественный сервис ☎️ +7 (495) 259-24-19',ENT_QUOTES)));
} else {
    $document->setTitle(strip_tags(html_entity_decode(ucfirst($chast_2[0]).' сайдинг '.$pokr.' '.$tol.' мм. '.$cvet.' | ⚒️ Металл Профиль',ENT_QUOTES)));
	$document->setDescription(strip_tags(html_entity_decode(ucfirst($chast_2[0]).' сайдинг в покрытии '.$pokr.' '.$tol.' мм. цвет '.$cvet.' ➡️ купить от Металл Профиль ✅ доступные цены ➡️ Качественный сервис ☎️ +7 (495) 259-24-19',ENT_QUOTES)));
}
...
<div class="row">
	...
	<div class="tab-pane fade show active" id="tabs8" role="tabpanel" aria-labelledby="tabs8-tab">
		...
		if (in_array('5179', $arr_cat_id)) {
				echo $this->loadTemplate('text18');
			}
			elseif (in_array('5180', $arr_cat_id)) {
				echo $this->loadTemplate('text19');
			}
			elseif (in_array('5178', $arr_cat_id)) {
				echo $this->loadTemplate('text20');
			}			
		else{
